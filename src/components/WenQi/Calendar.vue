<template>
  ...
  >
    ...
    <div class="grid">
        ...
        >
          ...
          <div :class="['content', `text-color-${index + 1}`]">
            <div :class="info.special_events === '生日活動' ? 'booking-color' : ''">
    ...
    export default {
      ...
      const calendarInfo = data
        ...
        .map(item => {
          ...
          const bookingColor = item.special_events === '生日活動' ? '#880e4f' : '';
          return {
    ...
    this.fetchCalendarData();
        ...
        // 获取前(后)一个月的年月日信息
        getPreMonth() {
        let month = this.selectData.month - 1;
        let year = this.selectData.year;
        if (month < 1) {
          month = 12;
          year--;
        }
        ...
        // 切换上(下)一月
        handlePreMonth() {
          const prevMonthDate = this.getPreMonth();
        this.selectData.month = prevMonthDate.month;
        this.selectData.year = prevMonthDate.year;
        this.updateCalendarData();
        },
        handleNextMonth() {
          const nextMonthDate = this.getNextMonth();
        this.selectData.month = nextMonthDate.month;
        this.selectData.year = nextMonthDate.year;
        this.updateCalendarData();
        },
      ...
      updateSelectData() {
        const selectedDate = new Date(this.selectData.year, this.selectData.month - 1, 1);
        this.selectData.day = selectedDate.getDate();
