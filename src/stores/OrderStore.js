...
export const useOrderStore = defineStore("OrderStore", {
  ...
  getters: {
    ...
    filterByWorkingArea: (state) => (workingArea) => {
      return state.meal_info.filter(
        (meal) => meal.status === "未出餐" && meal.working_area === workingArea
      );
    },
    //依照訂單編號找出訂單
    findByOId() {
    ...
    getOrderInfo() {
      fetch("http://localhost:8080/order_info", {
        ...
        .then((data) => {
          data.sort(
            (a, b) =>
              new Date(a.lastmodified_time) - new Date(b.lastmodified_time)
          );
          this.order_info = data.slice(-5); //取最後存入的五筆資料
          this.order_info.sort(
            (a, b) =>
              new Date(b.lastmodified_time) - new Date(a.lastmodified_time)
          );
        });
    },
    //將訂單內容存入
